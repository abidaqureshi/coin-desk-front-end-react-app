...
Redux store
*/
import * as autocompleteActionTypes from './autocompleteActionTypes';

const makeQueryToTheServer = ( searchVal, codesList ) => {

    return ( dispatch ) => {
        
        const suggestedCodes = codesList.filter((codeObj)=>{
            const regex = new RegExp (searchVal,"i");
            if ( regex.test(codeObj.country) || regex.test(codeObj.currency) ) {
        ...
        dispatch({type:autocompleteActionTypes.UPDATE_SEARCH_RESULTS, payload:suggestedCodes})
...
const getCurrencyCodeList = () => {
    ...
    return ( dispatch ) => {
      ...
      /*
      As per currencies supported list api cors are not set by BI server and 
      its retruning about cors even i use axios or fetch so I had to made it 
      static from this url https://api.coindesk.com/v1/bpi/supported-currencies.json
      on bpi site.
      */

      const currencies = [{"currency":"AED","country":"United Arab Emirates Dirham"},
      ...
      dispatch({type:autocompleteActionTypes.GET_CURRENCY_CODE, payload:currencies});
...
const showCurrencyDetails = (code) => {
    ...
    return ( dispatch ) => {
            ...
            .then((response)=>{
                ...
                dispatch({type:autocompleteActionTypes.SHOW_CURRENCY_DETAILS, payload:response});
                return fetch(`https://api.coindesk.com/v1/bpi/historical/
                            close.json?currency=${code}`)

            }).then(response => response.json())
            .then((response)=>{

                dispatch({type:autocompleteActionTypes.SHOW_CURRENCY_MONTHLY_DETAILS, 
                            payload:response});
            }).catch(error=>{
